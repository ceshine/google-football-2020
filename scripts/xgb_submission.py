...
try:
    ...
    bst.load_model('/kaggle_simulations/agent/' + filename)
...
def agent(obs):
    ...
    if obs['ball_owned_team'] in (-1, 1):
        ...
        if e_dist > .03 and ball_targetz < 0.2:
        ...
        if (
            e_dist < .015 and ball_targetz < 0.2 and
    ...
    if (obs['ball_owned_team'] == 0 and obs["game_mode"] == GameMode.Normal.value) and inside(ball_projected, DEFEND_RANGE):
        ...
        # if obs["active"] == 0:
        #     return [Action.LongPass.value]
        if obs['left_team_direction'][obs['active']][0] <= 0 or random.random() < 0.3:
            return [random.choice([
                    Action.Top, Action.Bottom,
                    Action.Right, Action.BottomRight,
    ...
    probs = bst.predict(xgb.DMatrix(features))[0]
    if obs['ball_owned_team'] == 0 and random.random() < 0.3:
        predicted = np.random.choice(np.arange(14), p=probs)
    else:
        predicted = np.argmax(probs)  # , ntree_limit=bst.best_ntree_limit))
